...
function Messenger(socket) {
  ...
  // Set the reset pin
  //this.resetPin = hardware.digital[2];
...
Messenger.prototype.setFiltering = function(scan_policy, adv_policy, filter, callback) {
  this.execute(bglib.api.gapSetFiltering, [scan_policy, adv_policy, filter], callback);
}
...
Messenger.prototype.startAdvertising = function(userAdvData, callback) {
  var mode = userAdvData ? this.bgLib.GAPDiscoverableModes.gap_user_data : this.bgLib.GAPDiscoverableModes.gap_general_discoverable;
  this.execute(bglib.api.gapSetMode, [mode, this.bgLib.GAPConnectableMode.gap_directed_connectable], callback);
}
...
Messenger.prototype.deleteBond = function(peripheral, callback) {
...
Messenger.prototype.setOOBData = function(data, callback) {
  this.execute(bglib.api.smSetOOBData, [data], callback);
}

Messenger.prototype.setSecurityParameters = function(mitm, keysize, smpio, callback) {
  this.execute(bglib.api.smSetParameters, [mitm, keysize, smpio], callback);
}

Messenger.prototype.getSystemInfo = function(callback) {
  this.execute(bglib.api.systemGetInfo, [], callback);
}

Messenger.prototype.dfuReboot = function(intoDFU, callback) {
  this.execute(bglib.api.dfuReset, [intoDFU], callback);
}

Messenger.prototype.setFlashAddress = function(address, callback) {
  this.execute(bglib.api.dfuFlashSetAddress, [address], callback);
}

Messenger.prototype.flashUpload = function(data, callback) {
  this.execute(bglib.api.dfuFlashUpload, [data], callback);
}

Messenger.prototype.flashUploadFinish = function(callback) {
  this.execute(bglib.api.dfuFlashUploadFinish, [], callback);
}
// TODO:
...
Messenger.prototype.parseIncomingPackets = function(data) {
  ...
  var incomingPackets = this.bgLib.parseIncoming(data, function callbackSeeker(err, parsedPackets) {
    ...
    var parsedPackets_length = parsedPackets.length;

    //For each response
    for (var i = 0; i < parsedPackets_length; i++) {
          ...
          case 0x80:
            if (parsedPacket.response) {
              if (cClass === 0x00 && cID === 0x00) {
                self.emit("booted", parsedPacket.response);
              }
              ...
              else if (cClass === 0x09 && cID === 0x00) {
                self.emit('dfuReboot', parsedPacket.response);
              }
            }
              break;
